...
This instruction extracts, from the second source vector register, the Last active element, and then replicates that element to destructively fill the destination and first source vector.
...
```
if !IsFeatureImplemented(FEAT_SVE) && !IsFeatureImplemented(FEAT_SME) then EndOfDecode(Decode_UNDEF); constant integer esize = 8 << UInt(size); constant integer g = UInt(Pg); constant integer dn = UInt(Zdn); constant integer m = UInt(Zm); constant boolean isBefore = TRUE;
```
