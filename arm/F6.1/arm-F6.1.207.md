...
```
(FEAT_FP16) Applies when VRINTR{<c>}{<q>}.F16 <Sd>, <Sm>
```

## Encoding for the Single-precision scalar variant

```
Applies when (size == 10) VRINTR{<c>}{<q>}.F32
```
...
```
Applies when (size == 11) VRINTR{<c>}{<q>}.F64
```
...
```
(FEAT_FP16) Applies when (size == 01) VRINTR{<c>}{<q>}.F16 <Sd>, <Sm>
```
