...
```
Applies when (Q == 1) VRSHL{<c>}{<q>}.<dt> {<Qd>, }<Qm>, <Qn>
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); CheckAdvSIMDEnabled(); integer result; for r = 0 to regs-1 for e = 0 to elements-1 constant bits(esize) opelt = Elem[D[m+r], e, esize]; constant integer element = if unsigned then UInt(opelt) else SInt(opelt); integer shift = SInt(Elem[D[n+r], e, esize]<7:0>); if shift >= 0 then // left shift result = element << shift; else // rounding right shift shift = -shift; result = (element + (1 << (shift - 1))) >> shift; Elem[D[d+r], e, esize] = result<esize-1:0>;
```
