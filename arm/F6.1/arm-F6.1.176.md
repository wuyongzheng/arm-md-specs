...
```
if size == '11' then SEE "Related encodings"; if size == '00' || Vd<0> == '1' then constant boolean scalar_form = FALSE; constant integer d = UInt(D:Vd); constant integer n = UInt(N:Vn); constant integer m = UInt(M:Vm); constant integer esize = 8 << UInt(size); constant integer elements = 64 DIV esize; constant integer index = integer UNKNOWN;
```
...
```
if size == '11' then SEE "Related encodings"; if size == '00' || Vd<0> == '1' then UNDEFINED; constant boolean scalar_form = TRUE; constant integer d = UInt(D:Vd); constant integer n = UInt(N:Vn); constant integer m = if size == '01' then UInt(Vm<2:0>) else UInt(Vm); constant integer index = if size == '01' then UInt(M:Vm<3>) else UInt(M); constant integer esize = 8 << UInt(size); constant integer elements = 64 DIV esize;
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); CheckAdvSIMDEnabled(); integer op2; if scalar_form then op2 = SInt(Elem[Din[m],index,esize]); for e = 0 to elements-1 if !scalar_form then op2 = SInt(Elem[Din[m],e,esize]);
```

```
constant integer op1 = SInt(Elem[Din[n],e,esize]); // The following only saturates if both op1 and op2 equal -(2^(esize-1)) bits(2*esize) product; boolean sat; (product, sat) = SignedSatQ(2*op1*op2, 2*esize); Elem[Q[d>>1],e,2*esize] = product; if sat then FPSCR.QC = '1';
```