...
```
(FEAT_FP16) Applies when (size == 01) VCVTA{<q>}.<dt>.F16 <Sd>, <Sm>
```

## Encoding for the Single-precision scalar variant

```
Applies when (size == 10) VCVTA{<q>}.<dt>.F32 <Sd>, <Sm>
```
...
```
Applies when (size == 10) VCVTA{<q>}.<dt>.F32 <Sd>,
```
