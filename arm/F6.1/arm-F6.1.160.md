...
```
Applies when (Q == 0) VORR{<c>}{<q>}{.<dt>} {<Dd>, }<Dn>, <Dm>
```

## Encoding for the 128-bit SIMD vector variant

```
Applies when (Q == 1) VORR{<c>}{<q>}{.<dt>} {<Qd>, }<Qn>, <Qm>
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); CheckAdvSIMDEnabled(); for r = 0 to regs-1 D[d+r] = D[n+r] OR D[m+r];
```
