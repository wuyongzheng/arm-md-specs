...
```
if ConditionPassed() then EncodingSpecificOperations(); CheckCryptoEnabled32(); constant bits(128) X = Q[n>>1]; constant bits(128) Y = Q[d>>1]; constant bits(128) W = Q[m>>1]; constant boolean part1 = FALSE; Q[d>>1] = SHA256hash(X, Y, W, part1);
```
