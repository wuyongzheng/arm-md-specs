...
```
Applies when (Q == 0) VSWP{<c>}{<q>}{.<dt>} <Dd>, <Dm>
```

## Encoding for the 128-bit SIMD vector variant

```
Applies when (Q == 1) VSWP{<c>}{<q>}{.<dt>} <Qd>, <Qm>
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); CheckAdvSIMDEnabled(); for r = 0 to regs-1 if d == m then D[d+r] = bits(64) else D[d+r] = Din[m+r]; D[m+r] = Din[d+r];
```
