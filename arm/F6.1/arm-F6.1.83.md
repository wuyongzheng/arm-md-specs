...
```
Applies when (Q == 0) VEOR{<c>}{<q>}{.<dt>} {<Dd>, }<Dn>, <Dm>
```

## Encoding for the 128-bit SIMD vector variant

```
Applies when (Q == 1) VEOR{<c>}{<q>}{.<dt>} {<Qd>, }<Qn>, <Qm>
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); CheckAdvSIMDEnabled(); for r = 0 to regs-1 D[d+r] = D[n+r] EOR
```
