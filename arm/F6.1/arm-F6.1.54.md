...
```
(FEAT_FP16) Applies when (size == 01) VCMP{<c>}{<q>}.F16 <Sd>, <Sm>
```

## Encoding for the Single-precision scalar variant

```
Applies when (size == 10) VCMP{<c>}{<q>}.F32 <Sd>,
```
...
```
Applies when (size == 11) VCMP{<c>}{<q>}.F64 <Dd>,
```
...
```
Applies when (size == 10) VCMP{<c>}{<q>}.F32
```
...
```
(FEAT_FP16) Applies when (size == 01) VCMP{<c>}{<q>}.F16 <Sd>, #0.0
```

## Encoding for the Single-precision scalar variant

```
Applies when (size == 10) VCMP{<c>}{<q>}.F32 <Sd>, #0.0
```
