...
```
Applies when (Q == 1) VORR{<c>}{<q>}.I32 {<Qd>, }<Qd>, #<imm>
```

## Decode for all variants of this encoding

```
if cmode<0> == '0' || cmode<3:2> == '11' then SEE "VMOV (immediate)"; if Q == '1' && Vd<0> == '1' then UNDEFINED; constant bits(64) imm64 = AdvSIMDExpandImm('0', cmode, i:imm3:imm4); constant integer d = UInt(D:Vd); constant integer regs = if Q == '0' then 1 else 2;
```
...
```
Applies when (Q == 0) VORR{<c>}{<q>}.I16 {<Dd>, }<Dd>, #<imm>
```

## Encoding for the 128-bit SIMD vector variant

```
Applies when (Q == 1) VORR{<c>}{<q>}.I16 {<Qd>, }<Qd>, #<imm>
```
...
```
if ConditionPassed() then CheckAdvSIMDEnabled(); for r = 0 to regs-1 D[d+r] = D[d+r]
```

```
EncodingSpecificOperations(); OR imm64;
```
