...
```
constant integer d = UInt(Rd); constant integer t = UInt(Rt); constant integer n = UInt(Rn); if d == 15 || t == 15 || n == 15 then if d == n || d == t then UNPREDICTABLE;
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); constant bits(32) address = R[n]; if AArch32.ExclusiveMonitorsPass(address,2) then MemO[address, 2] = R[t]<15:0>; R[d] = ZeroExtend('0', 32); else R[d] = ZeroExtend('1', 32);
```
