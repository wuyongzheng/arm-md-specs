...
```
LDMDB{<c>}{<q>} <Rn>{!}, <registers> // (Preferred syntax) LDMEA{<c>}{<q>} <Rn>{!}, <registers> // (Alternate syntax, Empty Ascending stack)
```

## Decode for this encoding

```
constant integer n = UInt(Rn); constant bits(16) registers = register_list; constant boolean wback = (W == '1'); if n == 15 || BitCount(registers) < 1 then UNPREDICTABLE; if wback && registers<n> == '1' then UNPREDICTABLE;
```
...
```
EncodingSpecificOperations(); bits(32) address = bits(32) data; for i = 0 to 14 if registers<i> == '1' then if i != n then R[i] = MemS[address,4]; else data = MemS[address,4]; address = address + 4; if registers<15> == '1' then LoadWritePC(MemS[address,4]); if wback && registers<n> == '1' then R[n] = bits(32) UNKNOWN; if wback && registers<n> == '0' then
```

```
ConditionPassed() then R[n] - 4*BitCount(registers);
```

```
R[n] = R[n] - 4*BitCount(registers); if !wback && registers<n> == '1' then R[n] = data;
```
