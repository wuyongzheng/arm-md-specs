...
```
Applies when (Q == 0) VQDMULH{<c>}{<q>}.<dt> {<Dd>, }<Dn>, <Dm>
```
...
```
Applies when (Q == 1) VQDMULH{<c>}{<q>}.<dt> {<Qd>, }<Qn>, <Dm[x]>
```
...
```
Applies when (Q == 1) VQDMULH{<c>}{<q>}.<dt> {<Qd>, }<Qn>, <Qm>
```
...
```
Applies when (Q == 1) VQDMULH{<c>}{<q>}.<dt> {<Qd>, }<Qn>,
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); CheckAdvSIMDEnabled(); integer op2; if scalar_form then op2 = SInt(Elem[D[m],index,esize]); for r = 0 to regs-1 for e = 0 to elements-1 if !scalar_form then op2 = SInt(Elem[D[m+r],e,esize]); constant integer op1 = SInt(Elem[D[n+r],e,esize]); // The following only saturates if both op1 and op2 equal -(2^(esize-1)) bits(esize) result; boolean sat; (result, sat) = SignedSatQ((2*op1*op2) >> esize, esize); Elem[D[d+r],e,esize] = result; if sat then FPSCR.QC = '1';
```
