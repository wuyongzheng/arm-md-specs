...
```
CheckSVEEnabled(); constant integer VL = CurrentVL; constant bits(64) operand1 = if n == 31 then SP[64] else X[n, 64]; constant bits(64) result = operand1 + (imm * (VL DIV 8)); if d == 31 then SP[64] = result; else X[d, 64] = result;
```
