...
```
Applies when (Q == 0 && !(imm6[5:3] == 000 && L == 0)) VRSRA{<c>}{<q>}.<type><size> {<Dd>, }<Dm>, #<imm>
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); CheckAdvSIMDEnabled(); constant boolean round = TRUE; for r = 0 to regs-1 for e = 0 to elements-1 constant bits(esize) opelt = Elem[D[m+r],e,esize]; constant integer element = if unsigned then UInt(opelt) else SInt(opelt); constant integer result = RShr(element, shift_amount, round); Elem[D[d+r],e,esize] = Elem[D[d+r],e,esize] + result;
```
