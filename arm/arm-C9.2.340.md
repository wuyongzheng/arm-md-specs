## C9.2.340 UVDOT (2-way)

Multi-vector unsigned 16-bit integer vertical dot product by indexed element to 32-bit integer

This instruction computes the vertical dot product of the corresponding two unsigned 16-bit integer values held in the two first source vectors and two unsigned 16-bit integer values in the corresponding indexed 32-bit element of the second source vector. The widened dot product results are destructively added to the corresponding 32-bit element of the ZAsingle-vector groups.
...
```
UVDOT ZA.S[<Wv>, <offs>{, VGx2}], { <Zn1>.H-<Zn2>.H },
```
