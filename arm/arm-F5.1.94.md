...
```
if Rn == '1111' then SEE "LDRSB (literal)"; constant integer t = UInt(Rt); constant integer n = UInt(Rn); constant boolean postindex = FALSE; constant boolean add = TRUE; constant boolean register_form = FALSE; constant bits(32) imm32 = ZeroExtend(imm8, 32); constant integer m = integer UNKNOWN; // Armv8-A removes UNPREDICTABLE for R13 if t == 15 then UNPREDICTABLE;
```
...
```
constant bits(32) offset_addr = if add then (R[n] + offset) else (R[n] offset); constant bits(32) address = if postindex then R[n] else offset_addr; R[t] = SignExtend(MemU_unpriv[address,1], 32); if postindex then R[n] = offset_addr;
```
