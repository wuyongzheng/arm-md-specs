## C9.2.341 UVDOT (4-way)

Multi-vector unsigned integer vertical dot product by indexed element

This instruction computes the vertical dot product of the corresponding four unsigned 8-bit or 16-bit integer values held in the four first source vectors and four unsigned 8-bit or 16-bit integer values in the corresponding indexed 32-bit or 64-bit element of the second source vector. The widened dot product results are destructively added to the corresponding 32-bit or 64-bit element of the ZA single-vector groups.
...
```
UVDOT ZA.S[<Wv>, <offs>{, VGx4}], { <Zn1>.B-<Zn4>.B },
```
...
```
if !(IsFeatureImplemented(FEAT_SME2) && IsFeatureImplemented(FEAT_SME_I16I64)) then EndOfDecode(Decode_UNDEF); constant integer v = UInt('010':Rv); constant integer esize = 64; constant integer n = UInt(Zn:'00'); constant integer m = UInt('0':Zm); constant integer offset = UInt(off3); constant integer index = UInt(i1);
```
...
```
ZAvector[vec, VL] = result; vec = vec + vstride;
```
