...
```
if ConditionPassed() then EncodingSpecificOperations(); constant integer diff = UInt(R[n]<15:0>) UInt(R[m]<31:16>); constant integer sum = UInt(R[n]<31:16>) + UInt(R[m]<15:0>); R[d]<15:0> = diff<15:0>; R[d]<31:16> = sum<15:0>; PSTATE.GE<1:0> = if diff >= 0 then '11' else '00'; PSTATE.GE<3:2> = if sum >= \texttt{0x10000} then '11' else '00';
```
