...
```
Applies when (M == 0) SMLALD{<c>}{<q>} <RdLo>, <RdHi>, <Rn>, <Rm>
```

## Encoding for the SMLALDX variant

```
Applies when (M == 1) SMLALDX{<c>}{<q>} <RdLo>, <RdHi>, <Rn>, <Rm>
```
...
```
constant integer dLo = UInt(RdLo); constant integer dHi = UInt(RdHi); constant integer n = UInt(Rn); constant integer m = UInt(Rm); constant boolean m_swap = (M == '1'); if dLo == 15 || dHi == 15 || n == 15 || m == 15 then // Armv8-A removes UNPREDICTABLE for R13 if dHi == dLo then UNPREDICTABLE;
```
