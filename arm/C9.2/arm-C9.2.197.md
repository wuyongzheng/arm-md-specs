## C9.2.197 SDOT (2-way, multiple and indexed vector)

Multi-vector signed 16-bit integer dot product by indexed element to 32-bit integer

This instruction computes the dot product of two signed 16-bit integer values held in each 32-bit element of the two or four first source vectors and two signed 16-bit integer values in the corresponding indexed 32-bit element of the second source vector. The widened dot product result is destructively added to the corresponding 32-bit element of the ZA single-vector groups.
...
```
constant integer segmentbase = e - (e MOD eltspersegment); constant integer s = segmentbase + index; for i = 0 to 1 constant integer element1 = SInt(Elem[operand1, 2 * e + i, esize DIV 2]); constant integer element2 = SInt(Elem[operand2, 2 * s + i, esize DIV 2]); sum = sum + element1 * element2; Elem[result, e, esize] = sum; ZAvector[vec, VL] = result; vec = vec + vstride;
```
