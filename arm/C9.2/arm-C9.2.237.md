## C9.2.237 SQRSHR (four registers)
...
```
CheckStreamingSVEEnabled(); constant integer VL = CurrentVL; constant integer elements = VL DIV (4 * esize); bits(VL) result; for r = 0 to 3 constant bits(VL) operand = Z[n+r, VL]; for e = 0 to elements-1 constant bits(4 * esize) element = Elem[operand, e, 4 * esize]; constant integer res = (SInt(element) + (1 << (shift-1))) >> shift; Elem[result, r*elements + e, esize] = SignedSat(res, esize); Z[d, VL] = result;
```
