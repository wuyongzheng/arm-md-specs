...
```
Applies when (Q == 0) VAND{<c>}{<q>}{.<dt>} {<Dd>, }<Dn>, <Dm>
```
...
```
Applies when (Q == 1) VAND{<c>}{<q>}{.<dt>} {<Qd>, }<Qn>, <Qm>
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); CheckAdvSIMDEnabled(); for r = 0 to regs-1 D[d+r] = D[n+r] AND
```
