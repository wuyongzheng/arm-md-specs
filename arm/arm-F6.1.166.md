...
```
if ConditionPassed() then EncodingSpecificOperations(); CheckAdvSIMDEnabled(); bits(64) dest; constant integer h = elements DIV 2;
```

```
for e = 0 to h-1 bits(esize) op1elt = Elem[D[n],2*e,esize]; bits(esize) op2elt = Elem[D[n],2*e+1,esize]; integer element1 = if unsigned then UInt(op1elt) else SInt(op1elt); integer element2 = if unsigned then UInt(op2elt) else SInt(op2elt); integer result = if maximum then Max(element1, element2) else Min(element1, element2); Elem[dest,e,esize] = result<esize-1:0>; op1elt = Elem[D[m],2*e,esize]; op2elt = Elem[D[m],2*e+1,esize]; element1 = if unsigned then UInt(op1elt) else SInt(op1elt); element2 = if unsigned then UInt(op2elt) else SInt(op2elt); result = if maximum then Max(element1, element2) else Min(element1, element2); Elem[dest,e+h,esize] = result<esize-1:0>; D[d] = dest;
```
