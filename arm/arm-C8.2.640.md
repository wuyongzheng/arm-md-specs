...
```
CheckSVEEnabled(); constant integer VL = CurrentVL; constant integer elements = VL DIV esize; constant bits(VL) operand1 = Z[n, VL]; constant bits(VL) operand2 = Z[m, VL]; bits(VL) result; for e = 0 to elements-1 constant integer element1 = SInt(Elem[operand1, 2*e + 0, esize DIV 2]); constant integer element2 = SInt(Elem[operand2, 2*e + 0, esize DIV 2]); constant integer res = 2 * element1 * element2; Elem[result, e, esize] = SignedSat(res, esize); Z[d, VL] = result;
```
