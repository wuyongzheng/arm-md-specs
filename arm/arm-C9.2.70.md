...
Multi-vector single-precision convert to signed 32-bit integer, rounding toward zero

This instruction converts each element of the two or four source vectors from single-precision to the signed 32-bit integer nearer to zero, and places the results in the corresponding elements of the two or four destination vectors.
...
```
if !IsFeatureImplemented(FEAT_SME2) then constant integer n = UInt(Zn:'0'); constant integer d = UInt(Zd:'0'); constant integer nreg = 2; constant boolean unsigned = FALSE; constant FPRounding rounding = FPRounding_ZERO;
```
...
```
if !IsFeatureImplemented(FEAT_SME2) then constant integer n = UInt(Zn:'00'); constant integer d = UInt(Zd:'00'); constant integer nreg = 4; constant boolean unsigned = FALSE; constant FPRounding rounding = FPRounding_ZERO;
```
