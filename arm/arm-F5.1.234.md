...
```
if ConditionPassed() then EncodingSpecificOperations(); if PSTATE.EL == EL2 then UNPREDICTABLE; // Hyp mode constant bits(32) offset = (if register_form then Shift(R[m], shift_t, shift_n, PSTATE.C) else imm32); constant bits(32) offset_addr = if add then (R[n] + offset) else (R[n] offset); constant bits(32) address = if postindex then R[n] else offset_addr; MemU_unpriv[address,1] = R[t]<7:0>; if postindex then R[n] = offset_addr;
```
