...
```
if ConditionPassed() then EncodingSpecificOperations(); constant integer sum = SInt(R[n]<15:0>) + SInt(R[m]<31:16>); constant integer diff = SInt(R[n]<31:16>) SInt(R[m]<15:0>); R[d]<15:0> = sum<15:0>; R[d]<31:16> = diff<15:0>; PSTATE.GE<1:0> = if sum >= 0 then '11' else '00'; PSTATE.GE<3:2> = if diff >= 0 then '11' else '00';
```
