## C9.2.325 USDOT (4-way, multiple and indexed vector)

Multi-vector unsigned by signed 8-bit integer dot product by indexed element to 32-bit integer

This instruction computes the dot product of four unsigned 8-bit integer values held in each 32-bit element of the two or four first source vectors and four signed 8-bit integer values in the corresponding indexed 32-bit element of the second source vector. The widened dot product result is destructively added to the corresponding 32-bit element of the ZA single-vector groups.
...
```
constant integer segmentbase = e - (e MOD eltspersegment); constant integer s = segmentbase + index; for i = 0 to 3 constant integer element1 = UInt(Elem[operand1, 4 * e + i, esize constant integer element2 = SInt(Elem[operand2, 4 * s + i, esize sum = sum + element1 * element2; Elem[result, e, esize] = sum; ZAvector[vec, VL] = result; vec = vec + vstride;
```
