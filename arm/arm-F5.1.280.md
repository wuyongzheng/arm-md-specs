...
```
Applies when (S == 1) UMULLS{<c>}{<q>} <RdLo>, <RdHi>, <Rn>, <Rm>
```

## Encoding for the Not flag setting variant

```
Applies when (S == 0) UMULL{<c>}{<q>}
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); constant integer result = UInt(R[n]) * R[dHi] = result<63:32>; R[dLo] = result<31:0>; if setflags then PSTATE.N = result<63>; PSTATE.Z = IsZeroBit(result<63:0>); // PSTATE.C, PSTATE.V unchanged
```
