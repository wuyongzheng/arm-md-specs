## C9.2.43 BFMOPA (widening)

BFloat16 sum of outer products to single-precision, accumulating
...
```
constant integer a = UInt(Pn); constant integer b = UInt(Pm); constant integer n = UInt(Zn); constant integer m = UInt(Zm); constant integer da = UInt(ZAda);
```
