...
```
SXTB{<c>}{<q>} {<Rd>, }<Rm> {, ROR #<amount>} SXTB{<c>}.W {<Rd>, }<Rm> // (<Rd>, <Rm> can be represented in T1)
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); constant bits(32) rotated = ROR(R[m], R[d] = SignExtend(rotated<7:0>, 32);
```
