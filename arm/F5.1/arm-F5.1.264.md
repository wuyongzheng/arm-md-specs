...
```
Applies when (!(imm5 == 00000 && stype == 11)) TST{<c>}{<q>} <Rn>, <Rm> {, <shift> #<amount>}
```
...
```
TST{<c>}.W <Rn>, <Rm> // (<Rn>, <Rm> can be represented
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); bits(32) shifted; bit carry; (shifted, carry) = Shift_C(R[m], shift_t, shift_n, PSTATE.C); constant bits(32) result = R[n] AND shifted; PSTATE.N = result<31>; PSTATE.Z = IsZeroBit(result); PSTATE.C = carry; // PSTATE.V unchanged
```
