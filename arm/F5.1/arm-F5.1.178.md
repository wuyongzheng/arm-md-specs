...
```
Applies when SBCS{<c>}{<q>} {<Rd>, }<Rn>, <Rm>, RRX
```
...
```
Applies when (S == 1 && !(imm5 == 00000 && stype == 11)) SBCS{<c>}{<q>} {<Rd>, }<Rn>, <Rm> {, <shift> #<amount>}
```
...
```
SBC<c>{<q>} {<Rdn>, }<Rdn>, <Rm> // (InITBlock()) SBCS{<q>} {<Rdn>, }<Rdn>, <Rm> // (Outside IT block)
```
...
For the 'A1 SBC, rotate right with extend', 'A1 SBC, shift or rotate by value', 'A1 SBCS, rotate right with extend', and 'A1 SBCS, shift or rotate by value' variants: is the general-purpose destination register, encoded in the 'Rd' field. If omitted, this register is the same as &lt;Rn&gt; . Arm deprecates using the PC as the destination register, but if the PC is used:
...
- For the SBCS variant, the instruction performs an exception return, that restores PSTATE from SPSR\_&lt;current\_mode&gt;.

For the 'T2 SBC, rotate right with extend', 'T2 SBC, shift or rotate by value', 'T2 SBCS, rotate right with extend', and 'T2 SBCS, shift or rotate by value' variants: is the general-purpose destination register, encoded in the 'Rd' field. If omitted, this register is the same as &lt;Rn&gt; .

For the 'A1 SBC, rotate right with extend', 'A1 SBC, shift or rotate by value', 'A1 SBCS, rotate right with extend', and 'A1 SBCS, shift or rotate by value' variants: is the first general-purpose source register, encoded in the 'Rn' field. The PC can be used, but this is deprecated.

For the 'T2 SBC, rotate right with extend', 'T2 SBC, shift or rotate by value', 'T2 SBCS, rotate right with extend', and 'T2 SBCS, shift or rotate by value' variants: is the first general-purpose source register, encoded in the 'Rn' field.

## &lt;Rm&gt;

For the 'A1 SBC, rotate right with extend', 'A1 SBC, shift or rotate by value', 'A1 SBCS, rotate right with extend', and 'A1 SBCS, shift or rotate by value' variants: is the second general-purpose source register, encoded in the 'Rm' field. The PC can be used, but this is deprecated.

For the 'T1', 'T2 SBC, rotate right with extend', 'T2 SBC, shift or rotate by value', 'T2 SBCS, rotate right with extend', and 'T2 SBCS, shift or rotate by value' variants: is the second general-purpose source register, encoded in the 'Rm' field.
...
For the 'A1 SBC, shift or rotate by value' and 'A1 SBCS, shift or rotate by value' variants: is the shift amount, in the range 1 to 31 (when &lt;shift&gt; = LSL or ROR) or 1 to 32 (when &lt;shift&gt; = LSR or ASR) encoded in the 'imm5' field as &lt;amount&gt; modulo 32.

For the 'T2 SBC, shift or rotate by value' and 'T2 SBCS, shift or rotate by value' variants: is the shift amount, in the range 1 to 31 (when &lt;shift&gt; = LSL or ROR) or 1 to 32 (when &lt;shift&gt; = LSR or ASR), encoded in the 'imm3:imm2' field as &lt;amount&gt; modulo 32.
...
```
if ConditionPassed() then EncodingSpecificOperations(); constant bits(32) shifted = Shift(R[m], shift_t, shift_n, PSTATE.C); bits(32) result; bits(4) nzcv; (result, nzcv) = AddWithCarry(R[n], NOT(shifted), PSTATE.C); if d == 15 then // Can only occur for A32 encoding if setflags then ALUExceptionReturn(result); else ALUWritePC(result); else R[d] = result; if setflags then PSTATE.<N,Z,C,V> = nzcv;
```
