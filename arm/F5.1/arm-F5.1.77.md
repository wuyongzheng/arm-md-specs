...
```
Applies when (!(P == '0' && W == '1')) LDRB{<c>}{<q>} <Rt>, <label> // (Normal form) LDRB{<c>}{<q>} <Rt>, [PC, #{+/-}<imm>] // (Alternative form)
```

## Decode for this encoding

```
if P == '0' && W == '1' then SEE "LDRBT"; constant integer t = UInt(Rt); constant bits(32) imm32 = ZeroExtend(imm12, 32); constant boolean add = (U == '1'); constant boolean wback = (P == '0') || (W == '1'); if t == 15 || wback then UNPREDICTABLE;
```
...
```
LDRB{<c>}{<q>} <Rt>, <label> // (Preferred syntax) LDRB{<c>}{<q>} <Rt>, [PC, #{+/-}<imm>] // (Alternative syntax)
```
...
```
if Rt == '1111' then SEE "PLD"; constant integer t = UInt(Rt); constant bits(32) imm32 = constant boolean add = (U == '1'); // Armv8-A removes UNPREDICTABLE for R13
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); constant bits(32) base = Align(PC32,4); constant bits(32) address = if add then (base + imm32) else (base -R[t] = ZeroExtend(MemU[address,1], 32);
```
