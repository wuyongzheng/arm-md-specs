...
```
if ConditionPassed() then EncodingSpecificOperations(); constant integer sum1 = SInt(R[n]<15:0>) + SInt(R[m]<15:0>); constant integer sum2 = SInt(R[n]<31:16>) + SInt(R[m]<31:16>); R[d]<15:0> = SignedSat(sum1, 16); R[d]<31:16> = SignedSat(sum2, 16);
```