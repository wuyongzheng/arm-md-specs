...
```
if ConditionPassed() then EncodingSpecificOperations(); constant integer diff = UInt(R[n]<15:0>) constant integer sum = UInt(R[n]<31:16>) + R[d]<15:0> = UnsignedSat(diff, 16); R[d]<31:16> = UnsignedSat(sum, 16);
```
