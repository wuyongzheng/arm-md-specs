...
```
if ConditionPassed() then EncodingSpecificOperations(); constant integer sum = UInt(R[n]<15:0>) + UInt(R[m]<31:16>); constant integer diff = UInt(R[n]<31:16>) UInt(R[m]<15:0>); R[d]<15:0> = sum<16:1>; R[d]<31:16> = diff<16:1>;
```
