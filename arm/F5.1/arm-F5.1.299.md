...
```
UXTH{<c>}{<q>} {<Rd>, }<Rm> {, ROR #<amount>} UXTH{<c>}.W {<Rd>, }<Rm> // (<Rd>, <Rm> can be represented in T1)
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); constant bits(32) rotated = ROR(R[m], R[d] = ZeroExtend(rotated<15:0>, 32);
```
