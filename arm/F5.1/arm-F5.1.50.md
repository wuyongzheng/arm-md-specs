...
```
Applies when (S == 1 && !(imm5 == 00000 && stype == 11)) EORS{<c>}{<q>} {<Rd>, }<Rn>, <Rm> {, <shift> #<amount>}
```
...
```
Applies when (S == 1 && imm3 == 000 && Rd != 1111 && imm2 == 00 && stype == 11) EORS{<c>}{<q>} {<Rd>, }<Rn>, <Rm>, RRX
```
...
```
if Rd == '1111' && S == '1' then SEE "TEQ (register)"; constant integer d = UInt(Rd); constant integer n = UInt(Rn); constant integer m = UInt(Rm); constant boolean setflags = (S == '1'); SRType shift_t; integer shift_n; (shift_t, shift_n) = DecodeImmShift(stype, imm3:imm2); // Armv8-A removes UNPREDICTABLE for R13 if (d == 15 && !setflags) || n == 15 || m == 15 then
```
...
For the 'A1 EOR, rotate right with extend', 'A1 EOR, shift or rotate by value', 'A1 EORS, rotate right with extend', and 'A1 EORS, shift or rotate by value' variants: is the general-purpose destination register, encoded in the 'Rd' field. If omitted, this register is the same as &lt;Rn&gt; . Arm deprecates using the PC as the destination register, but if the PC is used:
...
For the 'T2 EOR, rotate right with extend', 'T2 EOR, shift or rotate by value', 'T2 EORS, rotate right with extend', and 'T2 EORS, shift or rotate by value' variants: is the general-purpose destination register, encoded in the 'Rd' field. If omitted, this register is the same as &lt;Rn&gt; .

For the 'A1 EOR, rotate right with extend', 'A1 EOR, shift or rotate by value', 'A1 EORS, rotate right with extend', and 'A1 EORS, shift or rotate by value' variants: is the first general-purpose source register, encoded in the 'Rn' field. The PC can be used, but this is deprecated.

For the 'T2 EOR, rotate right with extend', 'T2 EOR, shift or rotate by value', 'T2 EORS, rotate right with extend', and 'T2 EORS, shift or rotate by value' variants: is the first general-purpose source register, encoded in the 'Rn' field.

## &lt;Rm&gt;

For the 'A1 EOR, rotate right with extend', 'A1 EOR, shift or rotate by value', 'A1 EORS, rotate right with extend', and 'A1 EORS, shift or rotate by value' variants: is the second general-purpose source register, encoded in the 'Rm' field. The PC can be used, but this is deprecated.

For the 'T1', 'T2 EOR, rotate right with extend', 'T2 EOR, shift or rotate by value', 'T2 EORS, rotate right with extend', and 'T2 EORS, shift or rotate by value' variants: is the second general-purpose source register, encoded in the 'Rm' field.
...
For the 'A1 EOR, shift or rotate by value' and 'A1 EORS, shift or rotate by value' variants: is the shift amount, in the range 1 to 31 (when &lt;shift&gt; = LSL or ROR) or 1 to 32 (when &lt;shift&gt; = LSR or ASR) encoded in the 'imm5' field as &lt;amount&gt; modulo 32.

For the 'T2 EOR, shift or rotate by value' and 'T2 EORS, shift or rotate by value' variants: is the shift amount, in the range 1 to 31 (when &lt;shift&gt; = LSL or ROR) or 1 to 32 (when &lt;shift&gt; = LSR or ASR), encoded in the 'imm3:imm2' field as &lt;amount&gt; modulo 32.
