...
```
if ConditionPassed() then EncodingSpecificOperations(); constant integer diff1 = SInt(R[n]<15:0>) SInt(R[m]<15:0>); constant integer diff2 = SInt(R[n]<31:16>) SInt(R[m]<31:16>); R[d]<15:0> = diff1<15:0>; R[d]<31:16> = diff2<15:0>; PSTATE.GE<1:0> = if diff1 >= 0 then '11' else '00'; PSTATE.GE<3:2> = if diff2 >= 0 then '11' else '00';
```
