...
```
Applies when (P == 1 && W == 0) STRB{<c>}{<q>} <Rt>, [<Rn>, {+/-}<Rm>{,
```

## Encoding for the Post-indexed variant

```
Applies when (P == 0 && W == 0) STRB{<c>}{<q>} <Rt>, [<Rn>], {+/-}<Rm>{,
```

## Encoding for the Pre-indexed variant

```
Applies when (P == 1 && W == 1) STRB{<c>}{<q>} <Rt>, [<Rn>, {+/-}<Rm>{,
```
...
```
if P == '0' && W == '1' then SEE "STRBT"; constant integer t = UInt(Rt); constant integer n = UInt(Rn); constant integer m = UInt(Rm); constant boolean index = (P == '1'); constant boolean add = (U == '1'); constant boolean wback = (P == '0') || (W == '1'); SRType shift_t; integer shift_n; (shift_t, shift_n) = DecodeImmShift(stype, imm5); if t == 15 || m == 15 then UNPREDICTABLE; if wback && (n == 15 || n == t) then UNPREDICTABLE;
```
...
```
STRB{<c>}{<q>} <Rt>, [<Rn>, {+}<Rm>{, LSL #<imm>}] STRB{<c>}.W <Rt>, [<Rn>, {+}<Rm>] // (<Rt>, <Rn>, <Rm> can be represented in T1)
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); constant bits(32) offset = Shift(R[m], shift_t, shift_n, PSTATE.C); constant bits(32) offset_addr = if add then (R[n] + offset) else (R[n] offset); constant bits(32) address = if index then offset_addr else R[n]; MemU[address,1] = R[t]<7:0>; if wback then R[n] = offset_addr;
```
