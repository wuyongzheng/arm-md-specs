## F5.1.7 ADD, ADDS (register-shifted register)
...
```
Applies when (S == 0) ADD{<c>}{<q>} {<Rd>, }<Rn>, <Rm>, <shift> <Rs>
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); constant integer shift_n = UInt(R[s]<7:0>); constant bits(32) shifted = Shift(R[m], shift_t, shift_n, PSTATE.C); bits(32) result; bits(4) nzcv; (result, nzcv) = AddWithCarry(R[n], shifted, '0'); R[d] = result; if setflags then PSTATE.<N,Z,C,V> = nzcv;
```
