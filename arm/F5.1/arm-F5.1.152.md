...
```
if ConditionPassed() then EncodingSpecificOperations(); constant integer sum = SInt(R[n]<15:0>) + constant integer diff = SInt(R[n]<31:16>) R[d]<15:0> = SignedSat(sum, 16); R[d]<31:16> = SignedSat(diff, 16);
```
