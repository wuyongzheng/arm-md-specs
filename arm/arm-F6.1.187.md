...
```
Applies when (Q == 0) VQSHL{<c>}{<q>}.<dt> {<Dd>, }<Dm>, <Dn>
```

## Encoding for the 128-bit SIMD vector variant

```
Applies when (Q == 1) VQSHL{<c>}{<q>}.<dt> {<Qd>, }<Qm>, <Qn>
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); CheckAdvSIMDEnabled(); for r = 0 to regs-1 for e = 0 to elements-1 constant integer shift = SInt(Elem[D[n+r],e,esize]<7:0>); constant bits(esize) opelt = Elem[D[m+r],e,esize]; constant integer element = if unsigned then UInt(opelt) else SInt(opelt); boolean sat; bits(esize) result; if shift >= 0 then (result,sat) = SatQ(element << shift, esize, unsigned); else (result,sat) = SatQ(element >> -shift, esize, unsigned); Elem[D[d+r],e,esize] = result; if sat then FPSCR.QC = '1';
```
