...
```
Applies when (S == 1) SMLALS{<c>}{<q>} <RdLo>, <RdHi>, <Rn>,
```
...
```
Applies when (S == 0) SMLAL{<c>}{<q>} <RdLo>, <RdHi>, <Rn>, <Rm>
```
...
```
constant integer dLo = UInt(RdLo); constant integer dHi = UInt(RdHi); constant integer n = UInt(Rn); constant integer m = UInt(Rm); constant boolean setflags = FALSE; if dLo == 15 || dHi == 15 || n == 15 || m == 15 then UNPREDICTABLE; // Armv8-A removes UNPREDICTABLE for R13 if dHi == dLo then UNPREDICTABLE;
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); constant integer result = SInt(R[n]) * SInt(R[m]) + SInt(R[dHi]:R[dLo]); R[dHi] = result<63:32>; R[dLo] = result<31:0>; if setflags then PSTATE.N = result<63>; PSTATE.Z = IsZeroBit(result<63:0>); // PSTATE.C, PSTATE.V unchanged
```
