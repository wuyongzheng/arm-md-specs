## C8.2.57 BFMLALB (vectors)

BFloat16 multiply-add to single-precision (bottom)
...
```
!IsFeatureImplemented(FEAT_BF16)) then EndOfDecode(Decode_UNDEF); constant integer n = UInt(Zn); constant integer m = UInt(Zm); constant integer da = UInt(Zda); constant boolean op1_neg = FALSE;
```
