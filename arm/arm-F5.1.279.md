...
```
Applies when (S == 1) UMLALS{<c>}{<q>} <RdLo>, <RdHi>, <Rn>,
```
...
```
Applies when (S == 0) UMLAL{<c>}{<q>} <RdLo>, <RdHi>, <Rn>, <Rm>
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); constant integer result = UInt(R[n]) * UInt(R[m]) + UInt(R[dHi]:R[dLo]); R[dHi] = result<63:32>; R[dLo] = result<31:0>; if setflags then PSTATE.N = result<63>; PSTATE.Z = IsZeroBit(result<63:0>); // PSTATE.C, PSTATE.V unchanged
```
