...
This instruction determines the unsigned maximum of the same element numbers from each 128-bit source vector segment, and places each result into the corresponding element number of the 128-bit SIMD&amp;FP destination register. Inactive elements in the source vector are treated as zero.
