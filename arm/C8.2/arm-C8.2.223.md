...
```
if !HaveSVE2FP8FMA() constant integer n = UInt(Zn); constant integer m = UInt(Zm); constant integer da = UInt(Zda);
```
...
```
for e = 0 to elements-1 constant bits(8) element1 = Elem[operand1, 4 * e + 1, 8]; constant bits(8) element2 = Elem[operand2, 4 * e + 1, 8]; constant bits(32) element3 = Elem[operand3, e, 32]; Elem[result, e, 32] = FP8MulAddFP(element3, element1, element2, FPCR, Z[da, VL] = result;
```
