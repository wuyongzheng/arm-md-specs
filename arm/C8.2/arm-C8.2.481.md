...
```
CheckSVEEnabled(); constant integer VL = CurrentVL; constant integer elements = VL DIV esize; constant integer eltspersegment = 128 DIV esize; constant bits(VL) operand1 = Z[n, VL]; constant bits(VL) operand2 = Z[m, VL]; bits(VL) result; for e = 0 to elements-1 constant integer segmentbase = e - (e MOD eltspersegment); constant integer s = segmentbase + index; constant integer element1 = UInt(Elem[operand1, e, esize]);
```

```
constant integer element2 = UInt(Elem[operand2, s, esize]); constant integer res = element1 * element2; Elem[result, e, esize] = res<esize-1:0>; Z[d, VL] = result;
```
