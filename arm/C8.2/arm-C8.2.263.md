...
Floating-point round to 64-bit integer (predicated)

This instruction rounds to integral floating-point values that fit into a 64-bit integer size using the rounding mode that is determined by the FPCR from each active floating-point element of the source vector, and places the results in the corresponding elements of the destination vector.
...
```
FRINT64X <Zd>.<T>, <Pg>/M, <Zn>.<T>
```

## Decode for this encoding

```
if !IsFeatureImplemented(FEAT_SVE2p2) && !IsFeatureImplemented(FEAT_SME2p2) then EndOfDecode(Decode_UNDEF); constant integer esize = 32 << UInt(sz); constant integer g = UInt(Pg); constant integer n = UInt(Zn); constant integer d = UInt(Zd); constant integer intsize = 64; constant FPRounding rounding = FPRoundingMode(FPCR); constant boolean merging = TRUE;
```
...
```
FRINT64X <Zd>.<T>, <Pg>/Z, <Zn>.<T>
```

## Decode for this encoding

```
if !IsFeatureImplemented(FEAT_SVE2p2) && !IsFeatureImplemented(FEAT_SME2p2) then EndOfDecode(Decode_UNDEF); constant integer esize = 32 << UInt(sz); constant integer g = UInt(Pg); constant integer n = UInt(Zn); constant integer d = UInt(Zd); constant integer intsize = 64; constant FPRounding rounding = FPRoundingMode(FPCR); constant boolean merging = FALSE;
```
