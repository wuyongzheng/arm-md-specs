...
Signed saturating rounding shift right narrow by immediate to interleaved integer
...
```
constant integer res = (SInt(element) + (1 << (shift-1))) >> shift; Elem[result, 2*e + i, esize] = SignedSat(res, esize); Z[d, VL] = result;
```