...
```
CheckSVEEnabled(); constant integer VL = CurrentVL; constant integer PL = VL DIV 8; constant integer elements = VL DIV esize; constant bits(PL) mask = P[g, PL]; constant bits(VL) operand1 = Z[dn, VL]; constant bits(VL) operand2 = if AnyActiveElement(mask, esize) then Z[m, VL] else Zeros(VL); bits(VL) result; for e = 0 to elements-1 if ActivePredicateElement(mask, e, esize) then constant integer dividend = SInt(Elem[operand1, e, esize]); constant integer divisor = SInt(Elem[operand2, e, esize]); integer quotient; if divisor == 0 then quotient = 0; elsif (dividend < 0) == (divisor < 0) then quotient = Abs(dividend) DIV Abs(divisor); // same signs - positive result else quotient = -(Abs(dividend) DIV Abs(divisor)); // different signs - negative result Elem[result, e, esize] = quotient<esize-1:0>; else Elem[result, e, esize] = Elem[operand1, e, esize]; Z[dn, VL] = result;
```
