...
```
CheckSVEEnabled(); constant integer VL = CurrentVL; constant integer segments = VL DIV 128; constant integer elements = 128 DIV esize; constant bits(VL) operand1 = Z[n, VL]; constant bits(VL) operand2 = Z[m, VL]; bits(VL) result = Z[d, VL]; for s = 0 to segments-1 for e = 0 to elements-1 constant integer idx = UInt(Elem[operand2, s * elements + e, esize]); if idx < elements then Elem[result, s * elements + e, esize] = Elem[operand1, s * elements + idx, esize]; Z[d, VL] = result;
```
