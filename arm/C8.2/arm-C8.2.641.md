## C8.2.641 SQDMULLB (indexed)

Signed saturating doubling multiply by indexed element (bottom)

This instruction multiplies then doubles the even-numbered signed elements within each 128-bit segment of the first source vector and the specified element in the corresponding second source vector segment, and places the results in overlapping double-width elements of the destination vector register. Each result element is saturated to the double-width N-bit element's signed integer range -2 (N-1) to (2 (N-1) )-1.
