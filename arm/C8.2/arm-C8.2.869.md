...
```
CheckSVEEnabled(); constant integer VL = CurrentVL; constant integer elements = VL DIV (2 * esize); bits(VL) result; for e = 0 to elements-1 for i = 0 to 1 constant bits(VL) operand = Z[n+i, VL]; constant bits(2 * esize) element = Elem[operand, e, 2 *
```
...
```
constant integer res = (UInt(element) + (1 << (shift-1))) >> shift; Elem[result, 2*e + i, esize] = UnsignedSat(res, esize); Z[d, VL] = result;
```