...
```
constant integer dLo = UInt(RdLo); constant integer dHi = UInt(RdHi); constant integer n = UInt(Rn); constant integer m = UInt(Rm); if dLo == 15 || dHi == 15 || n == 15 || m == 15 then if dHi == dLo then UNPREDICTABLE;
```
...
```
constant integer dLo = UInt(RdLo); constant integer dHi = UInt(RdHi); constant integer n = UInt(Rn); constant integer m = UInt(Rm); if dLo == 15 || dHi == 15 || n == 15 || m == 15 then // Armv8-A removes UNPREDICTABLE for R13 if dHi == dLo then UNPREDICTABLE;
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); constant integer result = UInt(R[n]) * UInt(R[m]) + UInt(R[dHi]) + UInt(R[dLo]); R[dHi] = result<63:32>; R[dLo] = result<31:0>;
```
