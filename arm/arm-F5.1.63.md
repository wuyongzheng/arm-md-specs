...
```
constant integer t = UInt(Rt); constant integer n = UInt(Rn); if t == 15 || n == 15 then
```
...
```
if ConditionPassed() then AArch32.SetExclusiveMonitors(address, 2);
```

```
EncodingSpecificOperations(); constant bits(32) address = R[n]; R[t] = ZeroExtend(MemO[address, 2], 32);
```
