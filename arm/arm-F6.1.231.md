...
```
Applies when (Q == 0 && !(imm6[5:3] == 000 && L == 0)) VSLI{<c>}{<q>}.<size> {<Dd>, }<Dm>, #<imm>
```
...
```
(Q == 1 && !(imm6[5:3] == 000 && L == VSLI{<c>}{<q>}.<size> {<Qd>, }<Qm>, #<imm>
```
...
```
if ConditionPassed() then EncodingSpecificOperations(); CheckAdvSIMDEnabled(); constant bits(esize) mask = LSL(Ones(esize), shift_amount); for r = 0 to regs-1 for e = 0 to elements-1 constant bits(esize) shifted_op = LSL(Elem[D[m+r],e,esize], shift_amount); Elem[D[d+r],e,esize] = (Elem[D[d+r],e,esize] AND NOT(mask)) OR shifted_op;
```
