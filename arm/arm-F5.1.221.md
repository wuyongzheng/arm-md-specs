...
```
if ConditionPassed() then EncodingSpecificOperations(); constant bits(32) address = R[n]; if AArch32.ExclusiveMonitorsPass(address,1) then MemO[address, 1] = R[t]<7:0>; R[d] = ZeroExtend('0', 32); else R[d] = ZeroExtend('1', 32);
```
