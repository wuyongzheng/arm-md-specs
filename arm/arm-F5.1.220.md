...
```
if ConditionPassed() then EncodingSpecificOperations(); constant bits(32) address = R[n]; if AArch32.ExclusiveMonitorsPass(address,4) then MemO[address, 4] = R[t]; R[d] = ZeroExtend('0', 32); else R[d] = ZeroExtend('1', 32);
```
