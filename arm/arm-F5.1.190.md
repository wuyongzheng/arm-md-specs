...
```
if ConditionPassed() then EncodingSpecificOperations(); constant integer sum = SInt(R[n]<15:0>) + SInt(R[m]<31:16>); constant integer diff = SInt(R[n]<31:16>) SInt(R[m]<15:0>); R[d]<15:0> = sum<16:1>; R[d]<31:16> = diff<16:1>;
```
