...
Signed saturating shift right narrow by immediate to unsigned integer (bottom)
...
```
CheckSVEEnabled(); constant integer VL = CurrentVL; constant integer elements = VL DIV (2 * esize); constant bits(VL) operand = Z[n, VL]; bits(VL) result; for e = 0 to elements-1 constant bits(2*esize) element = Elem[operand, e, 2*esize]; constant integer res = SInt(element) >> shift; Elem[result, 2*e + 0, esize] = UnsignedSat(res, esize); Elem[result, 2*e + 1, esize] = Zeros(esize); Z[d, VL] = result;
```