## 6.3.73 SMMU\_S\_STRTAB\_BASE\_CFG

The SMMU\_S\_STRTAB\_BASE\_CFG characteristics are:
...
This register is present only when SMMU\_S\_IDR1.SECURE\_IMPL == 1. Otherwise, direct accesses to SMMU\_S\_STRTAB\_BASE\_CFG are RES0.

## Attributes

SMMU\_S\_STRTAB\_BASE\_CFG is a 32-bit register.
...
- -For example, selecting SPLIT == 6 ( 0b0110 ) causes StreamID[5:0] to be used to index the lowest level Secure Stream table and StreamID[LOG2SIZE - 1:6] to index the upper level table.
- Note: If SPLIT &gt;= LOG2SIZE, a single upper-level descriptor indicates one bottom-level Secure Stream table with 2 LOG2SIZE usable entries. The valid range of the L1STD.Span value is up to SPLIT + 1, but not all of this Span is accessible, as it is not possible to use a StreamID &gt;= 2 LOG2SIZE .
...
A transaction having a StreamID &gt;= 2 LOG2SIZE is out of range. Such a transaction is terminated with an abort and a C\_BAD\_STREAMID event is recorded if permitted by SMMU\_S\_CR2.RECINVSID.

## Accessing SMMU\_S\_STRTAB\_BASE\_CFG

SMMU\_S\_STRTAB\_BASE\_CFG is Guarded by SMMU\_S\_CR0.SMMUEN.

See SMMU\_STRTAB\_BASE for detailed behavior.

Accesses to this register use the following encodings:

Accessible at offset 0x8088 from SMMUv3\_PAGE\_0
