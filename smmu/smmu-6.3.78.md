## 6.3.78 SMMU\_S\_EVENTQ\_PROD

The SMMU\_S\_EVENTQ\_PROD characteristics are:
...
This register is present only when SMMU\_S\_IDR1.SECURE\_IMPL == 1. Otherwise, direct accesses to SMMU\_S\_EVENTQ\_PROD are RES0.

## Attributes

SMMU\_S\_EVENTQ\_PROD is a 32-bit register.
...
- A Secure Event queue overflow is indicated using this flag. This flag is toggled by the SMMU when a queue overflow is detected, if OVFLG == SMMU\_S\_EVENTQ\_CONS.OVACKFLG.
- This flag will not be updated until a prior overflow is acknowledged by setting SMMU\_S\_EVENTQ\_CONS.OVACKFLG equal to OVFLG.
...
QS == SMMU\_S\_EVENTQ\_BASE.LOG2SIZE, see SMMU\_S\_EVENTQ\_CONS.
...
- Event records discarded when the Secure Event queue is disabled, that is when SMMU\_S\_CR0.EVENTQEN == 0.
- A stalled faulting transaction, as stall event records do not get discarded when the queue is full or disabled.

## Accessing SMMU\_S\_EVENTQ\_PROD

This register is Guarded by SMMU\_S\_CR0.EVENTQEN and must only be modified when SMMU\_S\_CR0.EVENTQEN == 0.

See SMMU\_EVENTQ\_BASE for detailed behavior.

Accesses to this register use the following encodings:

Accessible at offset 0x80A8 from SMMUv3\_PAGE\_0
