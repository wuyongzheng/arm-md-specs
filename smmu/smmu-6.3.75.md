## 6.3.75 SMMU\_S\_CMDQ\_PROD

The SMMU\_S\_CMDQ\_PROD characteristics are:
...
This register is present only when SMMU\_S\_IDR1.SECURE\_IMPL == 1. Otherwise, direct accesses to SMMU\_S\_CMDQ\_PROD are RES0.

## Attributes

SMMU\_S\_CMDQ\_PROD is a 32-bit register.
...
Bit [QS]: WR\_WRAP - Secure Command queue write index wrap flag.
...
QS == SMMU\_S\_CMDQ\_BASE.LOG2SIZE, see SMMU\_S\_CMDQ\_CONS.
...
Note: In the limit case of a one-entry queue, an increment of WR consists solely of a toggle of WR\_WRAP.

There is space in the queue for additional commands if:

```
SMMU_S_CMDQ_CONS.RD != SMMU_S_CMDQ_PROD.WR || SMMU_S_CMDQ_CONS.RD_WRAP == SMMU_S_CMDQ_PROD.WR_WRAP
```
...
A write to this register causes the SMMU to consider the Command queue for processing if SMMU\_S\_CR0.CMDQEN == 1 and SMMU\_S\_GERROR.CMDQ\_ERR is not active.

## Accessing SMMU\_S\_CMDQ\_PROD

Accesses to this register use the following encodings:

Accessible at offset 0x8098 from SMMUv3\_PAGE\_0
