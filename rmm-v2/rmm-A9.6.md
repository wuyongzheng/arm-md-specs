...
- IGXJLB The Realm discovers an assigned PCIe TDI by probing PCIe config space. Arm expects that this will be emulated by the Host, via Unprotected IPA space.
...
- IKNHFM A Realm is expected to retrieve cached device attestation evidence from the Host via an RSI\_HOST\_CALL interface. For details, refer to Realm Host Interface specification [20].
- IWBRKD A Realm can retrieve digests of the most recent device attestation evidence received by the RMM, by executing the RSI\_VDEV\_GET\_INFO command.
...
- IGWRDH A Realm device interface report describes the memory regions of the device. Each device memory region has the following attributes in the report:
...
- ILXTVZ A Realm can validate by execution of RSI\_VDEV\_VALIDATE\_MAPPING that each MMIO region with IS\_NON\_TEE\_MEM=0 in the Realm device interface report has been correctly mapped into the Realm's Protected IPA space.
- SJCXMF In order to determine the IPA to pass into RSI\_VDEV\_VALIDATE\_MAPPING, the Realm uses the contents of the device interface report to calculate an offset which it adds to the virtual BAR address.
...
- UPFCTN Arm recommends that the RMM should compute the MMIO Reporting Offset for a given VDEV using the following formula: MRO = x * 2^n
...
- INHQTB PCIe classifies memory locations into the following categories:
...
- ICWTNT A Realm can enable device DMA by calling RSI\_VDEV\_DMA\_ENABLE.
- ICDBSK The input values of RSI\_VDEV\_DMA\_ENABLE include the lock nonce, measurement sequence and report sequence values retrieved from the earlier call to RSI\_VDEV\_GET\_INFO. These are used to ensure that the configuration of the device has not changed between verification of device attestation evidence, and enablement of device DMA.
- ICPMFZ RSI\_VDEV\_DMA\_ENABLE fails unless the state of the VDEV is VDEV\_STARTED.
