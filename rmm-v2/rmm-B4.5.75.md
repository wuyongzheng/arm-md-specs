...
```
ram_invalid whose state
```

```
pre: ripas == RIPAS_RAM and command encounters RTT entry is neither RTTE_DATA nor RTTE_VOID. post: (result.status == RMI_ERROR_RTT && result.data.level.level == walk.level)
```
