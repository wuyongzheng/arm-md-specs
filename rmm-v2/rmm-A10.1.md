...
- IGRFHM The number of auxiliary Planes is specified by the Host at Realm creation.
- DZDRPY Planes within a Realm are identified using a zero-based Plane index .

The Plane index of the primary Plane is zero.

- DBVDMM When referring to the primary Plane of a Realm, this specification uses the term Plane 0 , or P0 .

When referring to any auxiliary Plane, this specification uses the term Pn .

- IJXZMT All Planes within a Realm share a single IPA space.

Stage 2 memory access permissions for a given IPA can differ between Planes.

IFMMRN Each Plane has a VMID which is unique both within the owning Realm and among all Realms.

- IVSJZZ A Realm with multiple Planes may either have:
- An RTT tree per Plane, or
- A single RTT tree, with per-Plane access permissions being managed indirectly.
- IBFJCV On REC exit due to Data Abort or Instruction Abort, the index of the RTT tree used by the exited Plane is provided to the Host.

This allows the Host to know which RTT tree must be modified in order to service a fault.

- DYTDGK A given VPE executes in one Plane at a time.

This is referred to as the active Plane of the VPE.

- IQXPKK The following capabilities are available only to P0:
