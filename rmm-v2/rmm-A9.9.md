...
- RJMMMP Access from a VDEV to an IPA is subject to the same stage 2 address translation used for PE accesses.
- IJGTYM A VDEV whose DMA state is VDEV\_DMA\_ENABLED is associated with a specified Plane within the Realm.
- ISVSNG Access from a VDEV to a Protected IPA follows the same rules, regarding stage 2 access permissions and stage 2 fault handling, as for a PE access from the associated Plane to the same IPA.
- IPXMVS A device may implement an Address Translation Cache (ATC), which it uses to store the result of Address Translation Service (ATS) requests issued to the SMMU.
- ILDNFN A device which implements an ATC may issue translated requests. In order to enforce isolation between Realms, translated requests are subject to checking by the SMMU against a Device Permission Table (DPT). The DPT is indexed by physical address, and stores the VMID which is permitted to access the corresponding physical location.
- IHRWYF The attributes of a Realm include the following which relate to device translated requests:
...
- INTMMR The Realm can discover the values of the 'ATS enable' and 'ATS Plane' attributes via the RSI\_REALM\_CONFIG command.
- IQRNTW When executing the RSI\_VDEV\_DMA\_ENABLE command, the Realm provides two parameters:
- An 'ATS enable' flag.
- A 'non-ATS Plane' index.
- RQRQHB If both the Realm 'ATS enable' flag and the VDEV 'ATS enable' flags are set then the device uses the stage 2 translation associated with the Realm 'ATS Plane'.

Otherwise, the device uses the stage 2 translation associated with the 'non-ATS Plane'.

DRAFT

- RKTSKL The STE.EATS flag is set on an SMMU stream if all of the following are true:
