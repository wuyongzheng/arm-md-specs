...
The RMI\_VDEV\_START command operates on the following context.
...
| vdev_state      | pre: post:   | vdev.vdev_state != VDEV_LOCKED result.status == RMI_ERROR_DEVICE        |
| comm_state      | pre: post:   | vdev.comm_state != DEV_COMM_IDLE result.status == RMI_ERROR_DEVICE      |

```
DRAFT pre: post: pre: post: B4.5.90.2.1 Failure condition ordering [rd_bound, rd_state, vdev_bound, vdev_gran_state] < [vdev_realm] [feat] < [rd_align, rd_bound, rd_state, pdev_align, pdev_bound, pdev_gran_state, vdev_align, vdev_bound, vdev_gran_state, vdev_realm] [vdev_gran_state] < [vdev_pdev, vdev_state, comm_state]
```
